...
namespace Coffee.UIExtensions
{
    ...
    public class UIDissolve : UIEffectBase, IMaterialModifier
    {
        ...
        // Serialize Members.
        //################################
        [SerializeField] [Range(0, 0.5f)] float m_Location = 0f;
        ...
        void Update()
        {
            ...
            if (flag == false)
            {
                _time += m_UpdateMode == AnimatorUpdateMode.UnscaledTime
                    ? Time.unscaledDeltaTime
                            : Time.deltaTime;//* UnityEngine.Random.Range(0.1f, 0.5f);
                location = (_time / m_Duration);

                if (m_Duration <= _time * 1.5f)
                {
                    //m_Play = false;
                    _time = 0;
                    flag = true;
            ...
            else
            {
                _time += m_UpdateMode == AnimatorUpdateMode.UnscaledTime
                    ? Time.unscaledDeltaTime
                            : Time.deltaTime;//* UnityEngine.Random.Range(0.1f, 0.5f);
                location = 0.5f - (_time / m_Duration);
                if (m_Duration <= _time)
                {
                    m_Play = false;
                    _time = 0;
